...
- [ ] `git rm` で `src/http.rs`、`src/server.rs` を削除。`Cargo.toml` の `[dependencies]` を空にし、
      `async-std` / `arti-client` / `tor-rtcompat` / `futures-util` を消す。`cargo update` で lock を再生成。
- [ ] `src/main.rs`、`src/config.rs`、`src/relay.rs` を b79359b 時点の同期版(std::io)に戻す。
      `git show b79359b:src/relay.rs` などで取得できる。
- [ ] `Cargo.toml` に以下を追加(rustc のメモリを増やさない設定):
      ```toml
      [profile.release]
      debug = 0
      [profile.dev]
      debug = 0
      ```
- [ ] README を SOCKS5 前提に書き換える(`--socks5-hostname` / `ALL_PROXY=socks5h://` の例)。
- 完了条件: `cargo build` が依存ゼロで通る(まだ何もしない main で可)。

### M1. OpenSSL FFI 層(`src/ffi/`)

- [ ] `#[link(name = "ssl")]` と `#[link(name = "crypto")]` を付けた `extern "C"` ブロックを書く。
      必要な関数は §4 の一覧。型は `*mut c_void` で十分(opaque)。
- [ ] `Drop` で必ず `*_free` を呼ぶ安全ラッパを作る。生ポインタを外に漏らさない。
- [ ] `SslStream`:
...
- [ ] `RAND_bytes`。
- [ ] 単体テスト: SHA-256("abc")、HMAC の RFC 4231 ベクタ、AES-128-CTR の NIST SP800-38A F.5.1、
...
- [ ] セル形式(tor-spec/cell-packet-format.md):
...
- [ ] ハンドシェイク手順(tor-spec/negotiating-channels.md):
  1. TLS 接続。
...
- [ ] CERTS の検証(negotiating-channels.md「Authenticating the responder」):
...
- [ ] CREATE2(10): `HTYPE=0x0002 | HLEN=84 | HDATA`。CREATED2(11): `HLEN | HDATA`。
- [ ] CircID: リンク v4 ではコネクションの開始側(=自分)が **MSB=1** の値を選ぶ。0 は禁止。
- [ ] RELAY セル本体(509 バイト、tor-spec/relay-cells.md):
...
- [ ] RELAY_EARLY(9): EXTEND2 は必ず RELAY_EARLY で送る。1 回路あたり最大 8 個。
- [ ] DESTROY(4)受信で回路を閉じる。理由コードは tor-spec/tearing-down-circuits.md。
- [ ] 完了条件: 1 ホップ回路で RELAY_BEGIN_DIR(13)→ CONNECTED(4)まで到達する
      (次の M4 と合わせて検証してよい)。
...
- [ ] BEGIN_DIR 経由 HTTP: 1 ホップ回路 → RELAY_BEGIN_DIR(payload 空) → CONNECTED →
...
- [ ] 鍵証明書(dir-spec/creating-key-certificates.md): `dir-key-certificate-version 3`、`fingerprint`、
...
- [ ] コンセンサス署名(dir-spec/consensus-formats.md): `directory-signature sha256 <identity> <signing-key-digest>`。
...
- [ ] コンセンサスのパース(必要な行だけ): `r <nick> <id_b64> <date> <time> <ip> <orport> <dirport>`、
...
- [ ] 回路の再利用: 1 回路に複数ストリームを載せる。作成から 10 分または
      ストリームが失敗したら新しい回路を作る。回路作成の全体タイムアウトは 60 秒。
- [ ] 完了条件: 3 ホップ回路で RELAY_BEGIN → CONNECTED が成功する。

### M6. ストリームとフロー制御(`stream.rs`)

- [ ] RELAY_BEGIN(1): `"host:port\0"`(FLAGS は省略)。CONNECTED(4)、END(3、理由コードは
      tor-spec/closing-streams.md、空 payload は MISC 扱い)。
- [ ] RELAY_DATA(2): 最大 498 バイト。
- [ ] フロー制御(tor-spec/flow-control.md):
...
- [ ] `TorStream`: `Read`/`Write` を実装し、`relay::bidirectional` にそのまま渡せるようにする。
- [ ] 完了条件: 3 ホップ経由で `GET https://check.torproject.org/api/ip` の本文が取れ、
      100KB 超のダウンロード(SENDME が動く)が完走する。

### M7. SOCKS5 サーバ(`socks5.rs`, `main.rs`)

- [ ] greeting: `05 NMETHODS METHODS` → `05 00`(認証なし)。`00` が無ければ `05 FF` で切断。
- [ ] request: `05 01 00 ATYP ...`。ATYP=03(hostname)を主に扱う。ATYP=01(IPv4)は許可、
      ATYP=04(IPv6)は `05 08`(address type not supported)。CMD≠01 は `05 07`。
- [ ] 応答: 成功 `05 00 00 01 00000000 0000`、失敗は Tor の END 理由を SOCKS の REP に写像
      (EXITPOLICY→02 not allowed、RESOLVEFAILED→04 host unreachable、CONNECTREFUSED→05、他→01)。
- [ ] 接続ごとにスレッド。`SERVER_PORT` 必須は既存 `config.rs` のまま。
- [ ] 完了条件: `curl --socks5-hostname 127.0.0.1:$SERVER_PORT https://check.torproject.org/api/ip` が
      `"IsTor":true`。`ALL_PROXY=socks5h://...` でも同じ。

### M8. 仕上げ

- [ ] ログ: `TOR_LOG=debug|info|warn` 環境変数で制御(println ベースで十分)。
- [ ] 実行時 RSS の計測を README に記載(`/proc/<pid>/status` の VmHWM)。
- [ ] `cargo clippy -- -D warnings`、`cargo fmt`。
- [ ] README: 使い方、制限事項(匿名性は本家に劣る、onion 非対応)、必要ライブラリ。
