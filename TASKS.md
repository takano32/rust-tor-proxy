...
## 24. 速い経路の自動選択(2026-09-03、調査と実装)

「自動で速い経路を選べるか」という問いに対する調査と、その結果入れた機能。
計画にあった作業ではないので、経緯と根拠をここに残す。

### 24.1 まず計測した

実ネットワークで、回路を変えながら 8MB を落として調べた。

| 予測子 | 相関(n=16、同一ガード) |
|---|---|
| 回路 RTT(EXTEND2 → EXTENDED2)↔ スループット | Pearson **+0.07** / Spearman −0.08 |
| 出口のコンセンサス帯域 ↔ スループット | Pearson **+0.15** / Spearman +0.10 |

**速い経路を事前に見分ける指標は見つからなかった**。RTT の低い方を選んでも
411 → 421 KB/s(+2%)にしかならない。一方で、

- **ひどい経路は実在し、持続する**。ガードを変えて測ると 0 / 68 / 136 KB/s と、
  正常な 400〜1100 KB/s とは population が分かれる。
- **ガードの寄与が大きい**。連続 4 回で 402/203/667/194 のガードと
  771/1367/1123/1032 のガードがあった(約 4 倍)。
- **同じ回路の 2 回目は 1 回目より速い**。9/9 で、中央値 452 → 757 KB/s(1.67 倍)。
  輻輳窓の立ち上がりであり、短い転送の計測を支配する。§23 の「4 並列で 4.4 倍」も
  2MB 転送でこれを踏んでいる。

### 24.2 したがって「最適化」ではなく「外れ値の排除」にした

13 エージェントによる調査(仕様・C Tor/arti・本コード・攻撃面)の最重要の指摘は、
**計測値が敵の操作できる入力である**こと。特に「遅いから引き直す」は、敵対中継が
遅延を足すだけで引き直しを誘発でき、望む経路を引くまで繰り返させられる
(proposal 344 の adversary-induced circuit creation)。

そこで、順位づけを一切せず、**絶対的な下限を割ったものを捨てる**方式にした。
順位づけは敵に「わずかに速く見せる」勾配を与えるが、下限は「使い物にならない」以外の
利得を与えない。残ったものからは**ランダム**に選ぶ。

### 24.3 入れたもの

| 場所 | 内容 |
|---|---|
| `circuit.rs` | `Meter`(受信バイト・**実際に受信していた時間**)。`State` ではなく `Inner` の atomic に置く。プールのロックを持ったまま読むため、`send_order` が定める順序に対する逆辺を作らない |
| `circuit.rs` | `Circuit::probe()`: 終端ホップに BEGIN_DIR を出し、何か返れば生存。**拒否も生存の証拠**(回路が通ることが問いであって、中継が親切かではない) |
| `pool.rs` | `judge()`: 1MiB 以上・実受信 2 秒以上の上で 150 KB/s 未満なら `TooSlow`。応答しなければ `Unresponsive` |
| `pool.rs` | `circuit_for` は最小負荷の中から**ランダム**に選ぶ(従来は `min_by_key` で先頭固定) |
| `pool.rs` | 完成済み回路を 3 本常備(従来 1 本)。選ぶ余地と、悪い回路の即時の代替を作る |
| `client.rs` | 30 分以内に 3 回ダメな回路を出したガードは次点へ。**1 実行あたり 2 回まで** |

`TooSlow` の回路は**閉じない**。Tor は確立済みストリームを別回路へ移せないので、
閉じても壊れるだけであり、しかも同じ数字は「配信元が遅い」ときにも出る。
プールから外して次のリクエストを別へ回すのが、できる範囲の「切り替え」である。
`Unresponsive` は回路自体が死んでいるので閉じ、無限のハングをクライアントに
見えるエラーに変える。

### 24.4 レビューで潰した欠陥(6 観点 × 敵対検証、confirmed 33 件)

最初の実装には実バグがあり、多エージェントレビューで検出して直した。

| 欠陥 | 直し方 |
|---|---|
| `circuit_for` が 2 回走査していたため、間に他スレッドが状態を変えると候補が空になり `rand::below(0)` で **panic → プールの Mutex が poison → プロセス全体が死ぬ** | 1 回走査に統一。候補は最小値の抽出元と同一集合なので空になりえない |
| スループットを**壁時計**(最初〜最後のセル)で割っていたため、再利用される回路がすべて「遅い」と判定される | 実際に受信していた時間だけを積算(セル間隔 500ms 以下のみ計上) |
| 「読み手が待っている＝回路が死んでいる」としたが、このプロキシは `io::copy` で**常に**読み手が待っている。idle な SSH・long-poll をすべて殺す | 判定をやめ、**生存プローブ**に置換。実測で健全な回路 7 本すべてが 0.3 秒で応答、誤検知 0 |
| `retire()` の blame を 6 箇所中 5 箇所が捨てていた | blame を `State` に持たせ、呼び出し側が忘れられないようにした |
| 1 回の障害で全回路が同時に blame され、3 ストライクを即座に使い切る | 1 スイープにつきガード 1 ストライク |
| ガード降格が `mark_failed` を呼びチャネルを閉じるため、**利用者の通信中のストリームが全部切れる** | `stop_using` を追加。選ばれなくなるだけで既存回路は生き続ける |
| `Circuit::close()` をプールのロック中に呼んでおり、チャネルの送信キューで詰まると全体が止まる | 閉じる対象を集めてロック解放後に閉じる |
| 降格が 10 分後の `retry_primary_guard` で復活し、上限だけ消費される | `demoted_at` を追加。降格は 30 分持続し、接続失敗の再試行とは別物として扱う |
| 到達不能条件 `len() < 2 && len() >= MAX_GUARDS` (常に偽) | 削除。ガードの追加は `ensure_guard` が行う |

### 24.5 プールの本数はなぜこの値か

「3 本は少なすぎないか、4096 本くらい保持してバックグラウンドで面倒を見ればよいのでは」
という問いに対して測った。

| 設定 | VmHWM | スレッド |
|---|---|---|
| MAX_CIRCUITS=8 | 17.4MB | 10 |
| MAX_CIRCUITS=24 | 17.6MB | 18 |
| MAX_CIRCUITS=48 | 17.9MB | 27 |

**メモリは制約ではない**。1 回路あたり約 30kB で、数十本ならただ同然。
制約は 3 つある。

1. **中継側の DoS 防御**(dos-spec/overview.md、`DoSCircuit*`)。中継はクライアント
   アドレスごとに回路生成レートを制限する(C Tor の既定で毎秒 3・バースト 90、
   超過で 1 時間拒否)。回路の寿命は dirty から 10 分なので、4096 本を維持するには
   毎秒約 7 本の生成が要り、ガードに即座にブロックされる。しかも 1 回路は 3 つの
   中継が状態を持つので、これはボランティアの資源に対する濫用でもある。
2. **回路 1 本につきポンプスレッド 1 本**。数十本なら問題ないが、数千本は
   カーネル側のコストとスケジューリングで破綻する。1 スレッドで多数の回路を
   多重化する作りに変えれば別だが、それは本プロジェクトの「スレッド + ブロッキング
   I/O」という前提そのものの変更になる。
3. **持っていても品質は分からない**。§24.1 のとおり、使う前に良し悪しを予測する
   指標が無い。未計測の回路を 4096 本持っても、選ぶときはやはりランダムである。

したがって上げたのは中程度まで: `MAX_CIRCUITS` 8 → 16、`CLEAN_TARGET` 3 → 6、
`STUB_TARGET` 2 → 3。あわせて**ビルダーを並列化**した(`BUILD_PARALLEL = 3`)。
本当にプールを薄くしていたのは本数の設定ではなく、1 本ずつ直列に建てていたこと
だった(48 本設定でも 75 秒で 20 本前後しか溜まらなかった)。並列化後は 90 秒で
回路 6 本 + stub 4 本に達し、VmHWM は 17.9MB のまま。

### 24.6 ファイルに残すもの、残さないもの

回路そのものは永続化できない(生きた TLS と鍵の状態で、プロセスと運命を共にする)。
永続化する価値があるのは「どの中継が良かったか」だが、**これは調査班が最も危険と
指摘したもの**である。出口は経路の最終ホップであり、自分の点数を自分で書ける。
速く見せることは安価で、合法的に見え、良い中継であることと区別できず、報酬は
このクライアントの将来のストリームの持続的な取り分になる。proposal 316 は、
複数の認証局と中央値投票で不正を防ごうとしていた Torflow ですら 89〜177 倍の
重み水増しを許したと記録している。単一クライアントの私的なファイルには、その
防御が 1 つも無い。

そこで永続化するのは**ガードの降格だけ**にした(`state/guards` に
`demoted <unix>` を追記)。ガードは最大 3 台で、利用者が明示的に速度優先を選んだ
位置であり、記録しないと「前回使い物にならないと判断したガードに、次回の 1 本目で
戻る」ことになる。古い形式のファイルもそのまま読める。

### 24.7 残る弱点

- **配信元が遅いだけの転送を回路のせいにしうる**。1 本の転送からは区別できない。
  代償は回路 1 本の廃棄(進行中の転送は続く)であり、3 回続けばガードも 1 回替わる。
- ガード交代の匿名性コストは本物で、利用者の明示的な選択によるもの。README に明記した。
- 「速い経路を選ぶ」ことは**していない**。できないと計測が示したため。
